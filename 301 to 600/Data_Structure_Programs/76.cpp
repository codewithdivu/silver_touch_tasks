...
private:
    ...
    Node* buildTree(vector<Point>& points, int dimension) {
        ...
        auto compare = (dimension == 0) ? [](const Point& a, const Point& b) { return a.x < b.x; }
    ...
    void rangeQuery(Node* node, int x1, int x2, int y1, int y2, int dimension, vector<Point>& result) {
        ...
        if (x1 <= node->key && node->key <= x2) {
            auto it = lower_bound(node->points.begin(), node->points.end(), Point(x1, y1),
                                       (dimension == 0) ? [](const Point& a, const Point& b) { return a.x < b.x; }
                                                       : [](const Point& a, const Point& b) { return a.y < b.y; });
